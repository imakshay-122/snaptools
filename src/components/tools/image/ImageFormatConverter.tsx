...
type ImageFormat = "image/jpeg" | "image/png" | "image/webp" | "image/gif" | "image/svg+xml";
...
const ImageFormatConverter = () => {
  ...
  const convertImageFormat = (dataUrl: string, format: ImageFormat) => {
    // If input is SVG and target is SVG, no conversion needed
    if (originalFormat === "image/svg+xml" && format === "image/svg+xml") {
    ...
    const img = new Image();
    img.onload = () => {
      // If target format is SVG, convert raster to SVG using path tracing
      if (format === "image/svg+xml") {
        ...
        setConvertedImage(`data:image/svg+xml;base64,${btoa(svgString)}`);
        return;
      }
